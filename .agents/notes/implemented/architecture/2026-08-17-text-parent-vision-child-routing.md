...
The browser plugin also projects each `vision/asset` event as a hidden direct-key Chat node and shadows the standard user and admitted-steering renderers. A model reference is displayed as an image only when its matching authorization event is present; otherwise the exact reference stays visible until history loading supplies the evidence. The renderer loads projected assets through the generated `visionLuna.read` Remote, whose Agent lookup and asset resolver recheck the rendered Session's authorization event before returning verified browser-only bytes. Native message images keep the standard Harness loader. The renderer removes verified internal references from the bubble and copy action without changing logged message content, so the parent still receives the text-only reference and replay remains reconstructable.
